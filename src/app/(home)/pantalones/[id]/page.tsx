...
const PantalonesId = () => {
    ...
    useEffect(() => {
        if (pantalonesId) {
            setSelectedImage(pantalonesId.image.src);
        }
    }, [pantalonesId]);
                ...
                <div className='flex flex-col md:flex-row gap-8 mb-20'>
                    <div className='w-auto m-auto 2xl:w-3/4 '>
                        <div className='flex flex-col-reverse md:flex-row items-center gap-3'>
                            <div className='flex flex-row md:flex-col gap-4'>
                                <Image onClick={() => handleChangeImage(pantalonesId.image)} width={95} height={120} src={pantalonesId.image} alt='img-remeras' className='cursor-pointer object-cover' />
                                <Image onClick={() => handleChangeImage(pantalonesId.image2)} width={95} height={120} src={pantalonesId.image2} alt='img-remeras' className='cursor-pointer object-cover' />
                                <Image onClick={() => handleChangeImage(pantalonesId.image3)} width={95} height={120} src={pantalonesId.image3} alt='img-remeras' className='cursor-pointer object-cover' />
                            </div>
                            <div className='m-auto'>
                                <Image onClick={handleZoomImage} width={450} height={550} src={selectedImage || pantalonesId.image} alt='img-remeras' className='cursor-pointer object-cover' />
                            </div>
                        </div>
                    </div>
                    ...
                    <div className='w-auto m-auto 2xl:w-1/4'>
                        <div className=' mb-10 lg:mb-20 max-w-xs'>
                            <p className='text-3xl text-[#303030] mb-1'>{pantalonesId.title}</p>
                            <div className='flex items-center gap-3 mb-1'>
                                <p className='text-2xl font-light text-[#303030]'>{formatPrice(pantalonesId.price)}</p>
                                <div className='flex items-center gap-1'>
                        ...
                        <div className='flex flex-col items-start gap-2 '>
                            <button onClick={() => handleBuyNow(pantalonesId)} className='py-2 px-6 rounded-md text-white bg-[#C171D6] duration-200 font-light w-[190px]'>Comprar ahora</button>
                            <button onClick={() => handleAddToCart(pantalonesId)} className='py-2 px-6 rounded-md text-white bg-[#79B4B7] duration-200 font-light w-[190px]'>Agregar al carrito</button>
                        </div>
                    </div>
                </div>
                ...
                <div className='mb-16'>
                    <div className='max-w-3xl'>
                        ...
                        <div className={`transition-all duration-300 w-full ${openMenu === 'description' ? 'max-h-screen opacity-100' : 'max-h-0 opacity-0 overflow-hidden'}`}>
                            {openMenu === 'description' &&
                                <p className='py-6'>{pantalonesId.description}</p>
                            }
                        </div>
                    </div>
