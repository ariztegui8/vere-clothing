'use client'
import CarrouselCamperas from '@/components/CarrouselCamperas'
...
const CamperasId = () => {
    ...
    const camperasId = dataCamperas.find((data) => data.id === Number(id));

    useEffect(() => {
        if (camperasId) {
            setSelectedImage(camperasId.image.src);
        }
    }, [camperasId]);
            ...
            <div className='pt-24 px-4 md:px-8 mb-10'>
                <div className='flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-10'>
                    <div className='flex items-center gap-1'>
                        ...
                        <Link href='/camperas'>
                            <p className='text-xs'>CAMPERAS /</p>
                        </Link>
                        <p className='text-xs text-[#79B4B7]'>campera {camperasId.id}</p>
                    </div>
                </div>
                ...
                <div className='flex flex-col md:flex-row gap-8 mb-20'>
                    <div className='w-auto m-auto 2xl:w-3/4 '>
                        <div className='flex flex-col-reverse md:flex-row items-center gap-3'>
                            <div className='flex flex-row md:flex-col gap-4'>
                                <Image onClick={() => handleChangeImage(camperasId.image)} width={95} height={120} src={camperasId.image} alt='img-remeras' className='cursor-pointer object-cover' />
                                <Image onClick={() => handleChangeImage(camperasId.image2)} width={95} height={120} src={camperasId.image2} alt='img-remeras' className='cursor-pointer object-cover' />
                                <Image onClick={() => handleChangeImage(camperasId.image3)} width={95} height={120} src={camperasId.image3} alt='img-remeras' className='cursor-pointer object-cover' />
                            </div>
                            <div className='m-auto'>
                                <Image onClick={handleZoomImage} width={450} height={550} src={selectedImage || camperasId.image} alt='img-remeras' className='cursor-pointer object-cover' />
                            </div>
                        </div>
                    </div>
                    ...
                    <div className='w-auto m-auto 2xl:w-1/4'>
                        ...
                        <div className='flex flex-col items-start gap-2 '>
                            <button onClick={() => handleBuyNow(camperasId)} className='py-2 px-6 rounded-md text-white bg-[#C171D6] duration-200 font-light w-[190px]'>Comprar ahora</button>
                            <button onClick={() => handleAddToCart(camperasId)} className='py-2 px-6 rounded-md text-white bg-[#79B4B7] duration-200 font-light w-[190px]'>Agregar al carrito</button>
                        </div>
                    </div>
                </div>
                ...
                <div className='mb-16'>
                    <div className='max-w-3xl'>
                        ...
                        <div className={`transition-all duration-300 w-full ${openMenu === 'description' ? 'max-h-screen opacity-100' : 'max-h-0 opacity-0 overflow-hidden'}`}>
                            {openMenu === 'description' &&
                                <p className='py-6'>{camperasId.description}</p>
                            }
                        </div>
                    </div>
